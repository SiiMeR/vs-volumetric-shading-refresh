...
public class ShadowTweaks
{
	...
	public ShadowTweaks(VolumetricShadingMod mod)
	{
		...
		_mod.CApi.Settings.AddWatcher<int>("volumetricshading_nearShadowBaseWidth", (OnSettingsChanged<int>)OnNearShadowBaseWidthChanged);
		_mod.CApi.Settings.AddWatcher<bool>("volumetricshading_softShadows", (OnSettingsChanged<bool>)OnSoftShadowsChanged);
		_mod.CApi.Settings.AddWatcher<int>("volumetricshading_softShadowSamples", (OnSettingsChanged<int>)OnSoftShadowSamplesChanged);
	...
	{
		if (!_softShadowsEnabled || !shader.includes.Contains("fogandlight.fsh") || ExcludedShaders.Contains(shader.PassName) || ShaderProgramBase.shadowmapQuality <= 0)
		{
			return;
		}
