...
public class YamlPatchLoader
{
	...
	public void LoadFromYaml(string yaml)
	{
		...
		foreach (PatchEntry item in new DeserializerBuilder().WithNamingConvention(CamelCaseNamingConvention.Instance).Build().Deserialize<IList<PatchEntry>>(yaml))
		{
			string content = item.Content;
			if (!string.IsNullOrEmpty(item.Snippet))
			{
				content = ((ICoreAPI)_capi).Assets.Get(new AssetLocation(_domain, "shadersnippets/" + item.Snippet)).ToText();
			}
			switch (item.Type)
			{
			case "start":
	...
	{
		RegexPatch regexPatch = ((patch.Filename == null) ? new RegexPatch(patch.Regex) : new RegexPatch(patch.Filename, patch.Regex));
