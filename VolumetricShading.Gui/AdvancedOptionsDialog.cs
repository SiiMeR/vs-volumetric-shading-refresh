...
public abstract class AdvancedOptionsDialog : GuiDialog
{
    ...
    {
        _isSetup = true;
        ElementBounds val = ElementStdBounds.AutosizedMainDialog.WithAlignment((EnumDialogArea)11)
            .WithFixedAlignmentOffset(0.0 - GuiStyle.DialogToScreenPadding, 0.0 - GuiStyle.DialogToScreenPadding);
        ...
        GuiComposer val6 = GuiComposerHelpers
            .AddDialogTitleBar(
                GuiComposerHelpers.AddShadedDialogBG(base.capi.Gui.CreateCompo(DialogKey, val), val5, true, 5.0, 0.75f),
                DialogTitle, (Action)OnTitleBarCloseClicked, (CairoFont)null, (ElementBounds)null)
            .BeginChildElements(val5);
        foreach (ConfigOption configOption in ConfigOptions)
        {
            ...
            {
                GuiComposerHelpers.AddSlider(val6, configOption.SlideAction, val3.FlatCopy().WithFixedWidth(200.0),
                    configOption.SliderKey);
            }
        ...
        foreach (ConfigOption item in ConfigOptions.Where((ConfigOption option) =>
                     option.SliderKey != null && !option.InstantSlider))
        {
